...
const UUID = process.env.UUID || 'a729cdf3-c5b9-4bcc-a8e6-2dddfb3d8513'; // 全协议共用UUID,不同平台建议修改,否则可能覆盖
const KOMARI_ENDPOINT = process.env.KOMARI_ENDPOINT || 'https://v.cws.kdns.fr';  // komari面板地址,例如：https://v.cws.kdns.fr
const KOMARI_TOKEN = process.env.KOMARI_TOKEN || 'GCHZ8L6lAkEiWtJn3zEm8S';        // komari节点Token,两个变量不全不安装监控端
const ARGO_DOMAIN = process.env.ARGO_DOMAIN || 'unikraftsin.jec.kdns.fr';          // 固定隧道域名,留空即启用临时隧道
const ARGO_AUTH = process.env.ARGO_AUTH || 'eyJhIjoiYmUzNmRiZDdkMjk2YTE5NjYxMjM2OWYxNGRmMGRjMDUiLCJ0IjoiOTE2OWFhYWItYjdjNy00MWYzLThhMmQtZDZkNzVjMTRiMzJiIiwicyI6Ik5HUTVORFE0WWprdE5qVXpOaTAwT0RJMkxXSTVNekF0WkdVeVptWm1NRFkyWXpVMSJ9';              // 固定隧道密钥json或token,留空即启用临时隧道,json获取地址：https://json.zone.id
...
const CHAT_ID = process.env.CHAT_ID || '673392065';                  // Telegram chat_id  两个变量不全不推送节点到TG 
const BOT_TOKEN = process.env.BOT_TOKEN || '8925915994:AAGLsnma8wX1jsbkF3uotm-bTCUnQIXe1K0';              // Telegram bot_token 两个变量不全不推送节点到TG 
...
function getFilesForArchitecture(architecture) {
  ...
  if (KOMARI_ENDPOINT && KOMARI_TOKEN) {
    ...
    baseFiles.unshift({
      fileName: phpPath,
      fileUrls: [`https://github.com/komari-monitor/komari-agent/releases/download/1.5.11/komari-agent-linux-${komariAsset}`]
    });
  }
