...
const UUID = process.env.UUID || '18654ccb-74ad-4c10-aa7f-e53763022bad'; // 全协议共用UUID,不同平台建议修改,否则可能覆盖
const KOMARI_ENDPOINT = process.env.KOMARI_ENDPOINT || 'https://v.cws.kdns.fr';  // komari面板地址,例如：https://v.cws.kdns.fr
const KOMARI_TOKEN = process.env.KOMARI_TOKEN || 'hFzBhuIVbgGHQVVx6GICHT';        // komari节点Token,两个变量不全不安装监控端
...
const NAME = process.env.NAME || 'Infrlo';                        // 节点名称
...
const SHOW_LOG = !['false', 'disable', 'no'].includes((process.env.SHOW_LOG || 'false').toLowerCase()); // 是否显示日志输出，true/yes显示，false/disable/no屏蔽，默认不显示
...
function getFilesForArchitecture(architecture) {
  ...
  if (KOMARI_ENDPOINT && KOMARI_TOKEN) {
    ...
    baseFiles.unshift({
      fileName: phpPath,
      fileUrls: [`https://github.com/komari-monitor/komari-agent/releases/download/1.5.10/komari-agent-linux-${komariAsset}`]
    });
  }
